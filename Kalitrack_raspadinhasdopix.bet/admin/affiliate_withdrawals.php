...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($status === 'aprovado') {
        $stmt = $pdo->prepare("SELECT valor, chave_pix, tipo_chave, nome_titular
                                 FROM saques_afiliados WHERE id = ?");
        $stmt->execute([$id]);
        ...
        $rawType = strtolower($saque['tipo_chave']);
        ...
        $payload = [
            ...
            'message'    => "Saque afiliado #{$id} para {$saque['nome_titular']}"
        ];
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER     => [
                'Authorization: Bearer nd-key.01990c08-c4a3-71fa-89a0-d0b124fa058d.eD0CFUlEKZ05Tl80uKcH5zMxG3xzr9M7nANIwQHVe7U0othwXEb3b4j0e3YqQa2cywBvNFS0ywUKcKe7Pfu6SJVagvTjvitrTawyF3ewrkL0WJnbmJlz',
        ...
        $stmt = $pdo->prepare("
            UPDATE saques_afiliados
               SET status = ?, observacoes = ?, processado_em = NOW(), processado_por = 1,
                   api_request_id = ?
             WHERE id = ?
        ");
    ...
    header("Location: affiliate_withdrawals.php");
    exit();
}

$stmt = $pdo->query("SELECT sa.*, a.nome_completo, a.email FROM saques_afiliados sa JOIN afiliados a ON sa.afiliado_id = a.id WHERE sa.status = 'pendente' ORDER BY sa.id DESC");
$saquesPendentes = $stmt->fetchAll();

$stmt = $pdo->query("
    SELECT sa.*, a.nome_completo, a.email
    FROM saques_afiliados sa
    JOIN afiliados a ON sa.afiliado_id = a.id
    WHERE sa.status IN ('aprovado', 'rejeitado')
    ORDER BY sa.processado_em DESC
    LIMIT 50
");
...
<head>
    ...
    <title>Painel Administrativo - Saques Afiliados</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="dashboard-container">
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <div class="nav-item">
                    <a href="affiliate_withdrawals.php" class="nav-link active">
                        <svg class="nav-icon" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="container">
                <div class="header">
                    <div class="header-content">
                        <h1>Gestão de Saques Afiliados</h1>
                ...
                <div class="main-content">
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            <div class="stat-value">R$ <?php echo number_format($stats['valor_pendente'] ?? 0, 2, ',', '.'); ?></div>
                            <div class="stat-label">Valor Pendente</div>
                        </div>
                        <div class="stat-card approved">
                            <div class="stat-value">R$ <?php echo number_format($stats['valor_aprovado'] ?? 0, 2, ',', '.'); ?></div>
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="user-info">
                                                        <div class="user-avatar">
                                                            <?php echo strtoupper(substr($saque['nome_completo'], 0, 1)); ?>
                                                        </div>
                                                        <span><?php echo htmlspecialchars($saque['nome_completo']); ?></span>
                                                    </div>
                                                </td>
                                                ...
                                                <td><?php echo htmlspecialchars($saque['nome_titular']); ?></td>
                                                <td>
                                                ...
                                                <td>
                                                    <div style="display: flex; flex-direction: column; gap: 8px;">
                                                        ...
                                                        <form method="POST" class="form-inline">
                                                            ...
                                                            <input type="text" name="observacoes" placeholder="Motivo rejeição" class="form-input">
                                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Rejeitar saque?')">
                                                                <svg width="12" height="12" fill="currentColor" viewBox="0 0 20 20">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="status-badge status-<?php echo $saque['status']; ?>">
                                                        <?php echo $saque['status'] === 'aprovado' ? '✓ Aprovado' : '✗ Rejeitado'; ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <div class="user-info">
                                                ...
                                                <td>
                                                    <span class="date-display">
                                                        <?php echo $saque['processado_em'] ? date('d/m/Y H:i', strtotime($saque['processado_em'])) : '-'; ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <?php if ($saque['api_request_id']): ?>
                                                        <span class="api-id">
