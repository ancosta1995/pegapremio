...
try {
    ...
    // Enviar evento de registro completo para tracking
    if ($click_id) {
        try {
            $result = sendTrackingEvent('CompleteRegistration', $click_id);
        } catch (Exception $e) {
            error_log("TRACKING ERROR: " . $e->getMessage() . " - Cadastro continuou normalmente");
        }
    }
