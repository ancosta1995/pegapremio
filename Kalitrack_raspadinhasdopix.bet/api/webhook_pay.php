...
// Paymaker envia paymentId como identificador da transação
$paymentId = $data['paymentId'] ?? $data['trans_id'] ?? $data['transaction_id'] ?? $data['chargeId'] ?? null;
if (!$paymentId) {
    error_log("ERRO: paymentId ausente no payload");
    error_log("Chaves disponíveis: " . implode(', ', array_keys($data)));
    http_response_code(400);
    echo json_encode(['error' => 'paymentId ausente']);
    exit;
}
...
// Paymaker usa diferentes status para pagamentos
$validPaymentStatuses = ['PAID', 'paid', 'approved', 'APPROVED', 'COMPLETED', 'completed'];
$validRefundStatuses = ['REFUNDED', 'refunded', 'CANCELLED', 'cancelled'];

if (!in_array($status, array_merge($validPaymentStatuses, $validRefundStatuses))) {
    error_log("INFO: Status '$status' não requer processamento - webhook ignorado");
...
try {
    // Primeiro tentar encontrar por transaction_id (trans_id da resposta da API)
    $stmt = $pdo->prepare("SELECT * FROM transactions WHERE transaction_id = ?");
    $stmt->execute([$paymentId]);
    $transacao = $stmt->fetch(PDO::FETCH_ASSOC);

    // Se não encontrar, tentar buscar por outros critérios (valor, email, etc.)
    if (!$transacao && isset($data['customer'])) {
        $customerEmail = $data['customer']['email'] ?? null;
        $totalValue = ($data['totalValue'] ?? 0) / 100; // Converter de centavos para reais
        
        if ($customerEmail && $totalValue > 0) {
            error_log("Tentando encontrar transação por email: $customerEmail e valor: $totalValue");
            
            // Buscar por email do usuário e valor aproximado (últimas 24 horas)
            $stmt = $pdo->prepare("
                ...
                LIMIT 1
            ");
            $stmt->execute([$customerEmail, $totalValue]);
            $transacao = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($transacao) {
                error_log("Transação encontrada por email e valor: " . $transacao['transaction_id']);
            }
        }
    }

    if (!$transacao) {
        error_log("ERRO: Transação não encontrada para PaymentId: $paymentId");
        
        // Log das últimas transações para debug
        $stmt = $pdo->prepare("SELECT transaction_id, user_id, username, amount, status, created_at FROM transactions ORDER BY created_at DESC LIMIT 5");
        $stmt->execute();
    ...
    if (in_array($status, $validPaymentStatuses)) {
        ...
        try {
            ...
            // Fallback para click_id se não existir
