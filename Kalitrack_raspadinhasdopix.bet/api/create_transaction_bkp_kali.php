...
function createValidEmail($username, $userId) {
    $cleanUsername = preg_replace('/[^a-zA-Z0-9]/', '',
...
function sendTrackingWebhook($eventData) {
    ...
    if (empty($webhookUrl)) {
        if (TRACKING_WEBHOOK_DEBUG) {
            SecurityHelper::logAPIAccess('tracking_webhook', 'DEBUG', 'URL do webhook não configurada - pulando envio');
        }
        return false;
    }
    ...
    $payload = [
        ...
        'ip_address' => SecurityHelper::getUserIP()
    ];
    ...
    if (TRACKING_WEBHOOK_DEBUG) {
        SecurityHelper::logAPIAccess('tracking_webhook', 'DEBUG', "Payload: $jsonPayload");
    }
    ...
    SecurityHelper::logAPIAccess('tracking_webhook', 'ATTEMPT', "Evento: {$eventData['evento']}, HTTP: $httpCode, Tempo: " . round($info['total_time'], 2) . "s");
    
    if ($curlError) {
        SecurityHelper::logAPIAccess('tracking_webhook', 'ERROR', "cURL: $curlError");
        return false;
    }
    
    if ($httpCode >= 200 && $httpCode < 300) {
        SecurityHelper::logAPIAccess('tracking_webhook', 'SUCCESS', "Evento: {$eventData['evento']}, User: {$eventData['user_id']}, Valor: R$ {$eventData['valor']}");
        return true;
    } else {
        SecurityHelper::logAPIAccess('tracking_webhook', 'ERROR', "HTTP $httpCode: $response");
        return false;
    }
}
...
function sendTrackingWebhookAsync($eventData) {
    try {
        ...
        SecurityHelper::logAPIAccess('tracking_webhook', 'ERROR', $e->getMessage());
        return false;
    }
}
...
try {
    ...
    try {
        ...
        if ($userTrackingData) {
            ...
            SecurityHelper::logAPIAccess('create_pix_transaction', 'TRACKING_WEBHOOK', "Evento gerouDep enviado - Resultado: " . ($webhookResult ? 'success' : 'failed'));
        } else {
